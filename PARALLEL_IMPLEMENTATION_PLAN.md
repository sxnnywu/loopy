...
1. **[C ← B]** C swaps the stub scorer → B's real Modal `score()`.  **WAITS ON:** B's `score()` deployed & callable. ✅ DONE — B's shared-scale `score_batch` landed (PR #9) and is wired into `score_test_gpu`; **live-validated on the A100** (test_5f5541fba6aa, IMG_7024 vs 7025): one batch session, shared 95th-pct scale, scores+winner written to Mongo, served complete via the deployed API. Shared scale differentiates peak/sustained/overall (0.7726 vs 0.7882 — real winner) where per-clip tied. ⚠️ Finding: `retention` stays 1.0/1.0 — the §3 ratio formula is scale-invariant AND clamped, so it saturates on any rising-engagement clip regardless of normalization; objective=`retention` winners are tie-break artifacts on such pairs. Demo: use objective `overall`; fix = §3 formula change (team). Deployed default stays SCORING_MODE=mock (A's integration clicks shouldn't spin A100s); flip at demo rehearsal.
...
3. **[A ← C]** A swaps `mock_api.json` → C's real endpoints.  **WAITS ON:** C's API live with real backends (1 & 2). ▶ **C's side fully unblocked** — API live at `https://jaychopra05--reeled-in-api.modal.run` (persistent, Atlas-backed, real D integrations); **CORS open for `https://reeled-in.base44.app`** (+ localhost dev) and **browser-path verified from the deployed Base44 page itself**: cross-origin `fetch()` + Authorization-headered request both 200 from A's origin — no Base44 CSP block (the §6 "CONFIRM at event" fetch risk is retired; Auth0 popup still to confirm once the tenant exists). Auth in dev-fallback until AUTH0_* set.
