...
- **Two+ variants → RANKING.** Show the winner, the per-network deltas (which system separated them), and surface the "why this ranking" rationale + citations from `SCORING_SCIENCE.md` beside the result (the ranking is only credible with the research shown). Per `SCORING_SCIENCE.md` §7 the ranking basis is the **real measured signals** — the 5 brain networks (available now) **plus** observable production signals (face/hands/speech/motion/clarity — B measures these but they are **not in the Score Object yet**; add them to §3 to complete this), **not** `peak/sustained/retention/overall`. The `GET /tests/{id}` response carries an **`analysis`** object (`mode`, `ranking`, `network_advantage`, `decisive_network`, `signal_advantage`) that computes this server-side. Full frontend build sheet: `VISUALIZATION_SPEC.md`.
- **Metrics are normalized proxy values in [0,1], NOT percentages.** Never render `retention: 1.0` as "100% retention" or `overall: 0.69` as a "69% grade." They are *directional proxy* scores (`SCORING_SCIENCE.md` §5); `retention` is a **ratio** of final-third ÷ first-third engagement clamped at 1.0 (1.0 = "held or grew," not "perfect"). Label them proxy scores and lead with the "directional proxy, not ground truth" caveat.
> **⚠️ Known metric issue (B to resolve):** the `retention` clamp at 1.0 saturates for *any* clip whose engagement rises over its runtime — so building clips all read 1.0 and can't be ranked on retention (reproduced live: 7024/7025 and 7026/7027 both 1.0/1.0). Until B amends the formula (§3), use `objective: overall` for comparison winners, and do not present `retention` as a standalone verdict.
