...
- **Two+ variants → RANKING.** Show the winner, the per-network deltas (which system separated them), and the metrics — **and** surface the "why this ranking" rationale + citations from `SCORING_SCIENCE.md` beside the result (the ranking is only credible with the research shown).
